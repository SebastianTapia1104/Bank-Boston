        ...
        do {
            ...
            for (Cuenta c : cliente.getCuentas()) {
                if (tipoCuenta == 1 && c instanceof CuentaCorriente) {
                    ...
                    break;
                } else if (tipoCuenta == 3 && c instanceof CuentaCredito) {
                    yaTieneCuentaDeTipo = true;
                    System.out.println("El cliente ya tiene una Cuenta de Crédito.");
                    break;
                }
            }
            ...
            if (tipoCuenta == 3) {
                ...
                if (!tieneCuentaBase) {
                    System.out.println("Para abrir una Cuenta de Crédito, el cliente debe tener previamente una Cuenta Corriente o de Ahorro.");
                    continue; 
                }
            }
            ...
            switch (tipoCuenta) {
                ...
                case 2:
                    ...
                    break;
                case 3: 
                    double sumaSaldosCuentasBase = 0.0;
                    for (Cuenta c : cliente.getCuentas()) {
                        if (c instanceof CuentaCorriente) {
                            sumaSaldosCuentasBase += c.getSaldo(); 
                        } else if (c instanceof CuentaAhorro) {
                            sumaSaldosCuentasBase += c.getSaldo(); 
                        }
                    }
                    double limiteCreditoCD = sumaSaldosCuentasBase * 0.75;
                    if (limiteCreditoCD <= 0) { 
                        System.out.println("La suma de saldos de sus cuentas base es muy baja para otorgar crédito.");
                        continue;
                    }
                    System.out.println("Límite de crédito calculado para la Cuenta de Crédito: $" + String.format("%.2f", limiteCreditoCD) + " CLP.");
                    nuevaCuenta = new CuentaCredito(numeroCuenta, limiteCreditoCD);
                    tipoCuentaValido = true;
                    break;
                default:
